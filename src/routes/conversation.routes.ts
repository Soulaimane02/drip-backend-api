...
import { verifyConversationOwnershipByUserIdMiddleware, verifyConversationOwnershipFromBodyMiddleware, verifyConversationOwnershipMiddleware } from "../middlewares/conversation.middlewares";
...
const conversationRoutes = () => {
  ...
  router.get("/user/:id", isSignedInMiddleware(), authorisationMiddleware([Role.Admin, Role.Seller, Role.User]), verifyConversationOwnershipByUserIdMiddleware(), conversationController.getConversationsByUserId.bind(conversationController));
