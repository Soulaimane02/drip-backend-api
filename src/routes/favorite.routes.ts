...
import { verifyFavoriteOwnershipByArticleIdMiddleware, verifyFavoriteOwnershipByFavoriteIdMiddleware, verifyFavoriteOwnershipByUserIdMiddleware, verifyFavoriteOwnershipMiddleware } from "../middlewares/favorite.middlewares";
...
const favoriteRoutes = () => {
  ...
  router.get("/user/:id", isSignedInMiddleware(), authorisationMiddleware([Role.Admin, Role.Seller, Role.User]), verifyFavoriteOwnershipByUserIdMiddleware(), favoriteController.getFavoritesByUserId.bind(favoriteController));
