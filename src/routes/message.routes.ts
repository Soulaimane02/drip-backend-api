...
import { verifyMessageOwnershipByConversationIdMiddleware, verifyMessageOwnershipCriticalMiddleware, verifyMessageOwnershipMiddleware, verifyMessageRespondOfferOwnershipMiddleware, verifyMessageSenderOwnershipMiddleware } from "../middlewares/message.middlewares";
...
const messageRoutes = (io: Server) => {
  ...
  router.put("/respond/:id", isSignedInMiddleware(), authorisationMiddleware([Role.Admin, Role.Seller, Role.User]), verifyMessageRespondOfferOwnershipMiddleware(), messageController.respondOffer.bind(messageController));
